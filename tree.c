...
t_node *create_all_Node(int nb_possibilities, int depth, t_move* possibilities, t_localisation robot, t_map map){   //La fonction de prends pas on compte si on avançant de plus de 10 mètres  si on a déjà marché sur une crevasse
    if (depth > NB_choices) return NULL;                                                                //Si la profondeur est supérieur au nombre de choix, on return NULL
    ...
    for (int i = 0; i < nb_possibilities; i++) {
        ...
        if (isValidLocalisation(new_loc.pos, map.x_max, map.y_max)) {                                   //Si la position après le mouvement est valide, on créé les enfants

            t_move* new_possibilities = remove_possibility(possibilities, nb_possibilities, i); //On créé le nouveau tableau de possibilités on retirant la case du noeud qu'on va créé car on l'aura déjà utilisé et on a déjà stocker la position après le mouvement pour connaître le coût
        ...
        else{                                                                                           //Si la position après le mouvement est valide, on ne créé pas d'enfant à la case concernée
            node->sons[i] = NULL;
        }
    }
    return node;
